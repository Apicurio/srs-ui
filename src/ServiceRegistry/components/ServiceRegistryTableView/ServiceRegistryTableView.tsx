...
import {
  ServiceRegistryToolbar,
  ServiceRegistryToolbarProps,
...
const ServiceRegistryTableView: React.FC<ServiceRegistryTableViewProps> = ({
  ...
  const renderNameLink = (name: string, row: IRowData) => {
    return (
      <Link
        to={`${basename}/t/${row?.id}`}
        data-testid='tableRegistries-linkKafka'
      >
        {name}
      </Link>
    );
    ...
    serviceRegistryItems?.forEach((row: IRowData) => {
      ...
      tableRow.push({
        cells: [
          {
            title:
              status?.toLowerCase() !== RegistryStatusValue.Ready
                ? name
